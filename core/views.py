...
def index(requests):
    ...
    user_following_list = []
    feed = []
    user_following=FollowersCount.objects.filter(follower = requests.user.username)
    ...
    current_user= User.objects.filter(username=requests.user.username)
    ...
    return render(requests,'index.html',{"user_profile": user_profile,'posts':feed_lists, 'suggestion_username_profile_list':suggestion_username_profile_list[:4]})
